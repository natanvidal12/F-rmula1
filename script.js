    ...
    function fmtTime(ms){
      ...
      const s = Math.floor(ms/1000); const cs = ms%1000;
    ...
    window.addEventListener('keydown', e=>{
      if (["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"," ","Space","p","P","r","R"].includes(e.key)) e.preventDefault();
      keys.add(e.key);
    ...
    const track = {
      ...
      pixelPerM: 2.2,
      checkpoints: [0.25, 0.5, 0.75, 0.98], // for simple progress
    };

    function resetRace(){
      state.started=false; state.paused=false; state.time=0; state.lap=1; state.trackOffset=0; state.trackCurveT=0; car.speed=0; car.x=0.5; car.angle=0; rivals.length=0; spawnRivalsInitial();
      setStatus('Pressione ESPAÇO para largar');
    }

    function startRace(){ if(!state.started){ state.started=true; state.lastTs=performance.now(); setStatus('Boa corrida!'); } }
    function togglePause(){ state.paused=!state.paused; setStatus(state.paused? 'Jogo pausado' : ''); }
    ...
    function makeRival(rx, sy, sp){
      return { x: rx, s: sp, y: sy, w: 46, h: 88, color: `hsl(${Math.floor(rand(0,360))} 80% 60%)`, blink: 0 };
    }
    function maybeSpawnRival(){
      if (rivals.length<12 && Math.random()<0.02) rivals.push(makeRival(rand(.12,.88), -600, rand(240, 360)));
    }
    ...
    function update(dt){
      ...
      if (state.trackOffset >= lapPixels){
        ...
        if (state.started){
          // lap time
          if (state.time>0){ if (state.best===null || state.time<state.best) state.best = state.time; }
          state.time = 0; state.lap++;
          if (state.lap>state.totalLaps){
            state.started=false; car.speed=0; setStatus('🏁 Corrida finalizada! Pressione R para reiniciar');
          }
        }
      ...
      for (const r of rivals){
        r.y += (r.s - car.speed) * dt; // screen space
        r.blink = Math.max(0, r.blink - dt);
      }
      // remove gone rivals
      for (let i=rivals.length-1;i>=0;i--){ if (rivals[i].y > state.height + 120) rivals.splice(i,1); }
      maybeSpawnRival();
      ...
      for (const r of rivals){
        ...
        if (intersect(playerRect, rect)){
          // bounce + slowdown
          car.speed *= 0.5; r.y -= 24; r.blink=0.25; setStatus('Colisão!');
        }
      }
    ...
    function roadX(norm){ // 0..1 within road to canvas x, with curve
    ...
    function draw(){
      ...
      const topY = -200; const bottomY = h+200;

      // draw kerbs stripes
      const stripeH = 28; const kerbW = track.kerbW;
      ...
      // player car
      drawCar(roadX(car.x), car.y*h, car.width, car.height, '#00e0ff');
    ...
    function drawFinishLine(x0, x1, height){
      const w = x1-x0; const cell = 18; const cols = Math.ceil(w/cell); const rows = Math.ceil(height/cell);
      for(let cy=0; cy<rows; cy++){
        for(let cx=0; cx<cols; cx++){
          const x = x0 + cx*cell; const y = -height + cy*cell;
    ...
    function drawCar(x, y, w, h, color, blinkAlpha=0){
      ...
      roundRect(-w*0.22, -h*0.06, w*0.44, h*0.26, 6); ctx.fill();
      // front/rear wings
      ctx.fillStyle = 'rgba(255,255,255,.85)';
      roundRect(-w*0.6, -h*0.48, w*1.2, h*0.06, 6); ctx.fill();
      roundRect(-w*0.6, h*0.42, w*1.2, h*0.06, 6); ctx.fill();
      // wheels
      ctx.fillStyle = '#111';
      ...
      if (blinkAlpha>0){ ctx.globalAlpha = blinkAlpha; ctx.fillStyle = '#fff'; roundRect(-w/2, -h/2, w, h, r); ctx.fill(); ctx.globalAlpha=1; }
      ctx.restore();
    }
